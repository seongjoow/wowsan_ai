...
class CustomTimeSeriesDataset(Dataset):
    ...
    def _prepare_data(self, data):
        ...
        for i in range(len(data) - self.input_window - self.target_window + 1):
            X.append(data[i:i+self.input_window].values.flatten())
            y.append(data[i+self.input_window:i+self.input_window+self.target_window][target_col].values)
    ...
    def get_num_features(self):
        return self.X.shape[1] // self.input_window
...
def load_and_prepare_data(file_path, input_window, target_window, target_node, target_feature, test_size=0.2):
    ...
    # 데이터셋 생성
    train_dataset = CustomTimeSeriesDataset(train_df, input_window, target_window, target_node, target_feature)
    test_dataset = CustomTimeSeriesDataset(test_df, input_window, target_window, target_node, target_feature)
...
class TimeSeriesTransformer(nn.Module):
    ...
    def forward(self, src, return_attention=False):
        ...
        if return_attention:
            ...
            attention = self.transformer.layers[-1].self_attn.attention_weights
...
def visualize_attention(attention_weights, input_window, target_window, target_node, target_feature):
    plt.figure(figsize=(10, 8))
    sns.heatmap(attention_weights, cmap='viridis')
    plt.title(f'Attention Weights for {target_node}_{target_feature}')
    plt.xlabel('Input Timesteps')
    plt.ylabel('Output Timesteps')
    
    # Add labels for input and output timesteps
    plt.xticks(range(input_window), [f't-{i}' for i in range(input_window, 0, -1)])
    plt.yticks(range(target_window), [f't+{i}' for i in range(1, target_window+1)])
...
def main():
    ...
    # 모델 초기화
    input_dim = train_dataset.get_num_features() * input_window
    ...
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            batch_x = batch_x.to(device)
            output, attention = model(batch_x, return_attention=True)
            predictions.append(output.cpu().numpy())
            actual_values.append(batch_y.numpy())
    ...
    # 어텐션 시각화
    # 첫 번째 샘플의 어텐션 가중치를 시각화
    visualize_attention(attention_weights[0], input_window, target_window, target_node, target_feature)


    # # 어텐션 시각화
    # for i in range(5):
    #     visualize_attention(attention_weights[i], input_window, target_window, target_node, target_feature)
        
    # # 추론 결과 저장
    # np.save("predictions.npy", predictions_unscaled)
    # np.save("actual_values.npy", actual_values_unscaled)
    # print("Predictions saved to predictions.npy")
    # print("Actual values saved to actual_values.npy")
