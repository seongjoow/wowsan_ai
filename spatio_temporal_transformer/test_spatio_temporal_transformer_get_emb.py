...
def test_node_embeddings(model, test_loader, device, node_names):
    """
    노드 임베딩을 추출하고 기본 정보를 출력하는 함수
    ...
    # CPU로 변환하고 numpy 배열로 변환
    node_embeddings = node_embeddings.cpu().numpy()
    
    # 임베딩 정보 출력
    print(f"\nNode embeddings shape: {node_embeddings.shape}")
    print("\nDetailed embedding information for each node:")
    for i, node_name in enumerate(node_names):
        embedding = node_embeddings[0, i]  # 첫 번째 배치의 각 노드 임베딩
        print(f"\n{node_name}:")
        print(f"Mean: {embedding.mean():.4f}")
        print(f"Std: {embedding.std():.4f}")
        print(f"Min: {embedding.min():.4f}")
        print(f"Max: {embedding.max():.4f}")
        print(f"First few values: {embedding[:5]}")  # 처음 5개 값만 출력
...
if __name__ == "__main__":
    ...
    node_embeddings = test_node_embeddings(model, test_loader, device, node_names)