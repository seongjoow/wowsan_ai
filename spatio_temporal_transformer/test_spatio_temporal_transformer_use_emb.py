...
class SpatioTemporalTransformer(nn.Module):
    def __init__(self, 
        ...
        # 성능 지표와 임베딩을 결합하는 레이어
        self.fusion_layer = nn.Linear(d_model + d_embedding, d_model)
    ...
    def forward(self, x: torch.Tensor, precomputed_emb: torch.Tensor) -> torch.Tensor:
        ...
        precomputed_emb = precomputed_emb.unsqueeze(1)  # [batch, 1, d_model]
        assert precomputed_emb.size() == (batch_size, self.d_model), f"Expected embedding shape {(batch_size, self.d_model)}, got {precomputed_emb.size()}"
...
class ExperimentManager:
    ...
    def _move_batch_to_device(self, batch_data, batch_targets):
        """Move batch data and targets to the appropriate device"""
        # Move each tensor in the dictionary to device
        batch_data = {
            key: tensor.to(self.device)
            for key, tensor in batch_data.items()
        }
        return batch_data, batch_targets.to(self.device)
    ...
    def train_epoch(self) -> float:
        ...
        for batch_data, batch_targets in self.train_loader:
            ...
            batch_data, batch_targets = self._move_batch_to_device(batch_data, batch_targets)
    ...
    def visualize_attention(self, 
                         sample_data, # 이제는 딕셔너리 형태: {'metrics': tensor(...), 'embeddings': tensor(...)}
                         node_names: List[str],
        ...
        Args:
            sample_data: 딕셔너리 형태의 입력 데이터
                - metrics: shape [1, timesteps, nodes, features]
                - embeddings: shape [1, timesteps, nodes, d_embedding]
...
def prepare_data(file_path, input_timesteps, forecast_horizon, target_node, target_feature, 
                 selected_features=None, test_size=0.2, val_size=0.1, batch_size=32, d_embedding=64):
    """
    데이터 준비 및 전처리
...
def create_sample_dataset():
    ...
    # 성능 지표 목록
    metrics = ['Throughput', 'ResponseTime']
    
    # 데이터프레임 생성을 위한 딕셔너리
    data = {'date': dates}
    
    # 각 노드와 메트릭에 대한 시계열 데이터 생성
    np.random.seed(42)
    
    # 각 노드별로 별도의 임베딩 데이터프레임 생성
    embedding_dfs = {}
    
    for node in nodes:
        # 임베딩 데이터 생성 (64차원)
        embedding_data = np.random.randn(len(dates), 64).astype(np.float32)
        
        # 각 차원을 별도의 열로 저장
        for i in range(64):
            data[f'{node}_Embedding_{i}'] = embedding_data[:, i]
        
        # 성능 지표 데이터 생성
        for metric in metrics:
            # 기본 시계열 패턴 생성
            base = np.sin(np.linspace(0, 10*np.pi, len(dates)))
            # 노이즈 추가
            ...
            # ResponseTime의 경우 항상 양수값을 가지도록 조정
            if metric == 'ResponseTime':
                values = np.abs(values) + 1
                
            data[f'{node}_{metric}'] = values
...
def test_model():
    ...
    experiment_config = {
        ...
        'batch_size': 32,
        'num_epochs': 10,  # 테스트를 위해 에포크 수 감소
        'patience': 2,
        'learning_rate': 1e-3,
        'd_embedding': 64,
        'd_model': 128,
    ...
    (train_loader, val_loader, test_loader,
     node_names, feature_names) = prepare_data(
    ...
    model = SpatioTemporalTransformer(
        ...
        d_embedding=experiment_config['d_embedding'],
    ...
    # 노드 임베딩 추출 테스트
    print("\nTesting node embedding extraction...")
    node_embeddings = model.get_node_embedding(sample_data)
    print("Node embeddings shape:", node_embeddings.shape)
    print("Node embeddings for first batch:", node_embeddings[0])
